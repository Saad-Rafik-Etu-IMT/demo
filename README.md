...
## 🔄 Versions pour démo CI/CD

### Version V1 (version actuelle - limitée)
Les endpoints suivants sont **désactivés** pour simuler une version antérieure :
- ❌ `DELETE /api/v1/clients/{id}` - Suppression de clients
- ❌ `DELETE /api/v1/vehicles/{id}` - Suppression de véhicules  
- ❌ `PATCH /api/v1/contracts/{id}/cancel` - Annulation de contrats

### Version V2 (version complète)
Pour activer la V2, **décommenter** les endpoints dans :
